...
class ContextDBMemoryProvider(MemoryProvider):
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        try:
            ...
            for r in results:
                ...
                lines.append(f"[contextdb] (conf={int(conf*100)}%, src={src}) {content}")
    ...
    def sync_turn(self, user_message: str, assistant_message: str, **kwargs) -> None:
        ...
        try:
            ns = self.db.namespace(self.namespace, mode=self.mode)
            source_id = f"session:{self._current_session_id}" if self._current_session_id else "hermes"

            ns.write(
                content=user_message,
                source_id=source_id,
                metadata={"role": "user", "session_id": self._current_session_id}
            )
            ns.write(
                content=assistant_message,
                source_id=source_id,
                metadata={"role": "assistant", "session_id": self._current_session_id}
            )
        except Exception as e:
    ...
    def handle_tool_call(self, name: str, arguments: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if name == "contextdb_search":
                results = ns.retrieve(text=arguments["query"], top_k=arguments.get("top_k", 5))
                return "\n".join(
